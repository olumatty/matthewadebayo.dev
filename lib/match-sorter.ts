...
export default function search<T extends Record<string, any>>(
  ...
  const options: MatchSorterOptions<T> = {
    keys: keys.map((key) => ({ key, threshold: matchSorter.rankings.CONTAINS })),
  };

  const wordOptions: MatchSorterOptions<T> = {
  ...
  const searchesArray = Array.from(searches);
  const [firstWord, ...restWords] = searchesArray;
